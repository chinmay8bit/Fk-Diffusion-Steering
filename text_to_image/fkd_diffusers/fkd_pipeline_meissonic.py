...
class FKDMeissonic(DiffusionPipeline):
...
# FK Steering Change
def latent_to_decode(*, model, output_type, latents, batch_size, height, width):
    if output_type == "latent":
        output = latents
    else:
        needs_upcasting = model.vqvae.dtype == torch.float16 and model.vqvae.config.force_upcast

        if needs_upcasting:
            model.vqvae.float()

        output = model.vqvae.decode(
            latents,
            force_not_quantize=True,
            shape=(
                batch_size,
                ...
                model.vqvae.config.latent_channels,
            ),
        ).sample.clip(0, 1)
